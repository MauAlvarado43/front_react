...
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/app;sed -i \"s/run front_reactjs/run front_reactjs_$KEY/\" \"bin/setup.sh\""
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/app;sed -i \"s/exec front_reactjs/exec front_reactjs_$KEY/\" \"bin/setup.sh\""
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/app;sed -i \"s/front_reactjs/front_reactjs_$KEY/\" \"bin/docker/docker-compose-dev.yml\""
